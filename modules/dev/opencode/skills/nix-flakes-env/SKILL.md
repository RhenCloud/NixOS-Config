...
name: nix-flakes-env
description: |
  使用 Nix Flakes 创建和管理项目开发环境。涵盖 flake.nix 模板、多语言 devShell、Snowveil 集成、输入管理、direnv 配置等。
...
框架按约定发现 `hosts/`、`homes/`、`modules/`、`packages/` 与 `overlays/`。单树模块使用 `options.nix`、`default.nix`、`nixos.nix`、`home.nix` 区分共享接口、中性实现、NixOS 和 Home Manager 模块。
