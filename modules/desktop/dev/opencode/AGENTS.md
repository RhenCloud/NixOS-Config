...
- **包管理**: Nix Flakes + Cloud Nix Framework
...
### 添加新模块
- **Home Manager**: 创建 `modules/<scope>/<name>/home.nix`，Cloud Nix Framework 自动发现
- **NixOS**: 创建 `modules/<scope>/<name>/nixos.nix`，Cloud Nix Framework 自动发现
